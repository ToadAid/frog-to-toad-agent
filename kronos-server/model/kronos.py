...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_samples=False):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        if return_samples:
            # Desk patch (see ATTRIBUTION.md): per-sample paths (batch, sample_count,
            # seq_len, feat) BEFORE the mean-collapse — the desk derives quantile
            # bands and P(up) from the spread. Unmodified upstream returns the mean.
            return preds
